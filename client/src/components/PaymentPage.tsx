...
// declare function pushToDataLayer(payload:any):void;
...
const paymentFormSchema = z.object({
  paymentMethod: z
    .enum(["creditCard", "debitCard", "netBanking", "wallet"])
    .default("creditCard"),
...
const PaymentPage: React.FC = () => {
  ...
  if (!flight) {
    return (
      <p>
        No flight or seat selection data found. Please go back and select a
        flight.
      </p>
    );
  }
  ...
  const calculateTotalAmount = () => {
    const seatPrice = 50000; // Example price per seat
  ...
  // const paymentData = {
  //     PaymentMethod: data.paymentMethod, cardLast4,
  ...
  const handleCompletePayment = (data:PaymentFormValues) => {
    const bookingId = "BRI" + Math.floor(
      1000 + Math.random() * 9000
    ).toString();

    const cardLast4 = data.cardNumber ? data.cardNumber.slice(-4):undefined;
        ...
        >
          ...
          <div className="sticky bottom-0 pt-4 pb-4 bg-white flex flex-wrap gap-2 justify-between">
            ...
            <div className="flex gap-2">
              <Button
                type="button"
                onClick={form.handleSubmit(handleCompletePayment)}
