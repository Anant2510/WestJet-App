...
const SeatSelection: React.FC = () => {
  ...
  const handleSeatClick = (
    seatId: string,
    currentStatus: "available" | "unavailable" | "selected"
  ) => {
    ...
    // Push seat_selection event to the data layer
    const seatDetails = updatedSeats.find((seat) => seat.id === seatId);
    const seatType =
      seatId.endsWith("A") || seatId.endsWith("F") ? "Window" : "Aisle";
    const seatClass =
      seatId.startsWith("1") || seatId.startsWith("2") ? "Business" : "Economy";
    const isPaidSeat = seatClass === "Business"; // Example logic for paid seats
    const seatPrice = isPaidSeat ? 25.0 : 10.0; // Example pricing logic

    pushToDataLayer({
      event: "seat_selection",
      user: {
        user_id: localStorage.getItem("loggedInUser") || "guest",
        loyaltyTier: "Gold", // Example loyalty status
        logged_in: !!localStorage.getItem("loggedInUser"),
        cusomerType: "frequent-flyer", // Example customer type
      },
      flight: {
        flight_number: storedFlight.flightNumber,
        departure_airport: storedFlight.departure,
        arrival_airport: storedFlight.arrival,
        departure_date: storedFlight.departureTime?.toISOString().split("T")[0],
      ...
      seat_selection: {
        ...
        seat_type: seatType,
        cabin_class: seatClass,
        is_paid_seat: isPaidSeat,
        seat_price: seatPrice,
        currency: "USD",
      },
      booking: {
        passenger_count: selectedSeats.length, // Example passenger count
  ...
  const calculateTotalPrice = () => {
    ...
    return (
      businessClassSeats * numericValue * 3 + economyClassSeats * numericValue
    );
  };
  ...
  const renderSeat = (seat: {
    ...
    const numericValue = Number(storedFlight?.price?.replace(/[^0-9.-]+/g, ""));
    return (
        ...
        <Tooltip>
          ...
          <TooltipContent>
            {isUnavailable ? (
              ...
              <p>Business Class: ₹{(numericValue * 3).toLocaleString()}</p>
            ) : (
              <p>Economy Class: ₹{numericValue.toLocaleString()}</p>
            )}
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  return (
    <div className="seat-selection-container p-6 bg-white rounded-lg shadow-lg max-w-5xl mx-auto">
      ...
      <div className="flight-info p-4 bg-gray-50 rounded-lg mb-6">
        <div className="text-sm text-neutral-600 mb-8">
          <span className="font-medium">
            {storedFlight.airline} {storedFlight.flightNumber}
          </span>
          |
        </div>
        <div className="flex justify-between mb-8">
          <div>
            ...
            <div className="text-lg font-semibold">
              {storedFlight?.departureTime?.toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </div>
            <span className="ml-2">{storedFlight.departure}</span>
          </div>

          <div className="flex flex-col items-center">
            <div className="text-sm text-neutral-500">
              {storedFlight.duration}
            </div>
          ...
          <div>
            ...
            <div className="text-lg font-semibold">
              {storedFlight?.arrivalTime?.toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              })}
            </div>
            <span className="ml-2">
              {storedFlight.arrival}
              {storedFlight.arrival_airport_name
                ? ` (${storedFlight.arrival_airport_name})`
                : ""}
            </span>
          </div>
      ...
      <div className=" flex justify-end">
        <div className="seating-layout mb-6 w-2/3 ">
          ...
          <div className="airplane-body bg-neutral-50 p-6 rounded-lg overflow-auto max-h-[400px]">
            <div className="airplane-front flex justify-center mb-4 border-b border-dashed border-neutral-300">
            ...
            <div className="business-class mb-6 pb-4 border-b border-dashed border-neutral-300">
              <div className="text-center mb-3 text-sm font-medium text-indigo-600">
                Business Class
              </div>
              <div className="seats-grid">
                {renderSeatingLayout().slice(0, 2)}
              </div>
            </div>

            {/* Economy class section */}
            <div className="economy-class">
              <div className="text-center mb-3 text-sm font-medium text-blue-600">
                Economy Class
              </div>
      ...
      <div className="booking-summary bg-gray-50 p-4 rounded-lg mb-6 w-1/3">
      ...
      <div className="sticky bottom-0 pt-4 pb-4 bg-white flex justify-between">
        ...
        <Button
          ...
          className="bg-[#e10a0a] hover:bg-[#e10a0a]"
          size="lg"
        >
          Proceed to Payment
        </Button>
